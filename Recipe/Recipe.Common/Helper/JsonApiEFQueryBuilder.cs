...
namespace Recipe.Common.Helper
{
    ...
    public static class JsonApiEFQueryBuilder
    {
        ...
        static Dictionary<Filters, Func<Expression, Expression, BinaryExpression>> FilterExpressions;
        static JsonApiEFQueryBuilder()
        {
            FilterExpressions = new Dictionary<Filters, Func<Expression, Expression, BinaryExpression>>();
            FilterExpressions.Add(Filters.Equal, Expression.Equal);
            FilterExpressions.Add(Filters.GreaterThan, Expression.GreaterThan);
            FilterExpressions.Add(Filters.GreaterThanEqual, Expression.GreaterThanOrEqual);
            FilterExpressions.Add(Filters.LessThan, Expression.LessThan);
            FilterExpressions.Add(Filters.LessThanEqual, Expression.LessThanOrEqual);
            FilterExpressions.Add(Filters.NotEqual, Expression.NotEqual);
        ...
        public static IQueryable<T> GenerateQuery<T>(this IQueryable<T> query, JsonApiRequest request)
        {
            ...
            {
                query = query.GenerateFilterQuery(request.Filter[filter.Key], filter.Value);
            }
        ...
        public static IQueryable<T> GenerateFilterQuery<T>(this IQueryable<T> query, Dictionary<string, string> filters, Func<Expression, Expression, BinaryExpression> expression)
        {
            ...
            if (filters != null && filters.Keys.Count > 0)
            {
                foreach (var key in filters.Keys)
                {
                    ...
                    if (property != null)
                    {
                        propertyExpr = GetPredicateBodyForProperty(property, filters[key], param, expression);
                    }
        ...
        private static Expression GetPredicateBodyForProperty(PropertyInfo prop, string queryValue, ParameterExpression param, Func<Expression, Expression, BinaryExpression> expression)
        {
            ...
            if (propertyType == typeof(String))
            {
                try
                {
                    if (String.IsNullOrWhiteSpace(queryValue))
                    {
                        ...
                        expr = expression(propertyExpr, Expression.Constant(null));
                    }
                    else
                    {
                        Expression propertyExpr = Expression.Property(param, prop);
                        expr = expression(propertyExpr, Expression.Constant(queryValue));
                    }
                }
            ...
            else if (propertyType == typeof(Boolean))
            {
                ...
                expr = bool.TryParse(queryValue, out value)
                    ? GetPropertyExpression(value, prop, param, expression)
            ...
            else if (propertyType == typeof(Boolean?))
            {
                ...
                expr = GetPropertyExpression(value, prop, param, expression);
            }
            ...
            else if (propertyType == typeof(DateTimeOffset))
            {
                ...
                expr = DateTimeOffset.TryParse(queryValue, out value)
                    ? GetPropertyExpression<DateTimeOffset>(value, prop, param, expression)
                    : Expression.Constant(false);
            }
            else if (propertyType == typeof(DateTimeOffset?))
            {
        ...
        private static Expression GetPropertyExpression<T>(T value, PropertyInfo property, ParameterExpression param, Func<Expression, Expression, BinaryExpression> expression)
        {
            ...
            return expression(propertyExpr, castedConstantExpr);
        }
        ...
        public static JsonApiRequest GetJsonApiRequest(this HttpRequestMessage request)
        {
            if (request != null)
            {
                ...
                jsonAPIRequest.Filter[Filters.Equal] = request.ExtractFilters("filterequal.");
                jsonAPIRequest.Filter[Filters.NotEqual] = request.ExtractFilters("filternotequal.");
                jsonAPIRequest.Filter[Filters.GreaterThan] = request.ExtractFilters("filtergreater.");
                jsonAPIRequest.Filter[Filters.LessThan] = request.ExtractFilters("filterless.");
                jsonAPIRequest.Filter[Filters.GreaterThanEqual] = request.ExtractFilters("filtergreaterequal.");
                jsonAPIRequest.Filter[Filters.LessThanEqual] = request.ExtractFilters("filterlessequal.");
        ...
        public static Dictionary<string, string> ExtractFilters(this HttpRequestMessage request, string filterType)
        {
            ...
            foreach (var queryPair in queryPairs)
            {
                ...
                if (!queryPair.Key.ToLower().StartsWith(filterType))
                    continue;

                var filterField = queryPair.Key.Substring(filterType.Length); // Skip "filter."
    ...
    public class JsonApiRequest
    {
        ...
        {
            this.Filter = new Dictionary<Filters, Dictionary<string, string>>();
        }

        public List<string> Sort { get; set; }

        public Dictionary<Filters, Dictionary<string, string>> Filter { get; set; }
